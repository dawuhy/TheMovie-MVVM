...
protocol LocalDBServiceType {
    ...
    func addObserver(completion: @escaping (Result<Results<FavoriteMovieObject>, Error>) -> Void)
}
...
class RealmService: LocalDBServiceType {
    ...
    func addMovie(movie: Movie) {
        let movieObject = FavoriteMovieObject()
    ...
    func loadData() {
        listFavoriteMovie = realm.objects(FavoriteMovieObject.self)
    }
